...
use crate::db::sqlite::repository::{MonitorRepository, ResourceRepository, TelemetryRepository, MetricRepository};
...
impl<'a> TelemetryEngine<'a> {
    ...
    pub async fn run_collection_tick(&self, node: &str, proxmox_vmid: u32, vm_id: Uuid) -> Result<()> {
        ...
        for resource in resources {
            ...
            if resource.state != ResourceState::Monitored {
                debug!("Skipping collection for {}/{}: State is {:?}", resource.kind, resource.identifier, resource.state);
                continue;
            }

            // Find an ENABLED monitor for this resource
            let monitors = self.monitor_repo.get_by_resource_id(resource.id)?;
            let active_monitor = monitors.into_iter().find(|m| m.state == ConfigState::Enabled);
            ...
            if let Some(monitor) = active_monitor {
                ...
                if monitor.collection_type == "linux_service" || resource.kind == "service" {
                    self.collect_service(&resource, &monitor, node, proxmox_vmid).await?;
                }
            } else {
                debug!("Skipping collection for {}/{}: No ENABLED monitor found", resource.kind, resource.identifier);
            }
        }
    ...
    async fn collect_service(&self, resource: &crate::domain::resource::Resource, monitor: &crate::domain::monitor::Monitor, node: &str, proxmox_vmid: u32) -> Result<()> {
        let result = self.service_collector.collect(node, proxmox_vmid, &resource.identifier).await?;
        
        // Find the matching Metric definition (e.g. "service_status")
        let metric = self.metric_repo.get_by_monitor_and_name(monitor.id, "service_status")?;
        ...
        if let Some(metric) = metric {
            ...
            debug!("Collected telemetry for {}: {:?}", resource.identifier, t.observation);
        } else {
            warn!("No 'service_status' metric defined for monitor {}", monitor.id);
        }
