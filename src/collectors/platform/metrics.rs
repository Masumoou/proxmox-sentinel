...
pub(super) fn update_platform_metrics(
    ...
    for pool in zfs {
        let status = if pool.state == "ONLINE" {
            "ok"
        } else {
            "critical"
        };
        crate::exporter::prometheus::update_platform_health("zfs", &pool.name, status);
        crate::exporter::prometheus::update_platform_value(
            "zfs",
            &pool.name,
            "capacity_pct",
            pool.capacity_pct,
    ...
    for backup in backups {
        crate::exporter::prometheus::update_platform_health(
            "backup",
            &backup.vmid.to_string(),
            &backup.status,
        );
    ...
    let failed_tasks = tasks
        .iter()
        .filter(|t| {
            t.status.to_lowercase().contains("error") || t.status.to_lowercase().contains("fail")
        })
        .count();
    ...
    for snapshot in snapshots {
        let status = if snapshot.oldest_days.unwrap_or(0) > 0 {
            "info"
        } else {
            "ok"
        };
