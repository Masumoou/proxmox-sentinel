...
pub(super) async fn collect_snapshots(
    ...
    {
        let api_rows = match client
            .guest_snapshots(&guest.node, &guest.kind, guest.vmid)
            .await
        {
            Ok(rows) => rows,
            Err(e) => {
        ...
        let oldest_days = snapshots
            .iter()
            .filter_map(|s| s.created_ts.map(|ts| (now - ts) / 86400))
            .max();
        if oldest_days.unwrap_or(0) >= cfg.snapshot_warn_days as i64 {
            alerts.push(platform_alert(
                ...
                "warning",
                format!(
                    "Guest {} ({}) has snapshots older than {} days",
                    guest.name, guest.vmid, cfg.snapshot_warn_days
                ),
            ));
        }
