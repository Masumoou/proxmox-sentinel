...
impl Collector for LinuxServiceCollector {
    async fn collect(&self, node: &str, proxmox_vmid: u32, identifier: &str) -> Result<CollectorResult> {
        ...
        match self.pve_client.vm_agent_exec_shell(node, proxmox_vmid, &command).await {
            Ok(output) => {
            ...
            Err(e) => {
                ...
                warn!("QEMU Guest agent execution failed on VM {} for service {}: {}", proxmox_vmid, identifier, e);
