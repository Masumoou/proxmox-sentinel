...
mod tests {
    ...
    fn test_history_preservation_when_rule_deleted() {
        ...
        let alert_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM alerts WHERE id = 'alert1'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        ...
        let incident_count: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM incidents WHERE id = 'inc1'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        ...
        assert!(
            res.is_err(),
            "Hard deleting a rule with alerts should violate foreign key constraints"
        );
    }
    ...
    fn test_unknown_state_suppresses_alert() {
        ...
        let observation: String = conn
            .query_row(
                "SELECT observation FROM telemetry WHERE id = 'tel1'",
                [],
                |row| row.get(0),
            )
            .unwrap();
