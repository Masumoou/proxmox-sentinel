...
use crate::proxmox_api::{ProxmoxClient, GuestKind};
use crate::db::sqlite::repository::{ResourceRepository, DiscoveryEventRepository};
...
impl<'a> DiscoveryEngine<'a> {
        ...
        for guest in guests {
            if guest.status == "running" && guest.kind == GuestKind::Vm {
                ...
                if let Err(e) = self.scan_guest(vm_id, guest.vmid, node).await {
                    warn!("Failed to discover services for VM {} on {}: {}", guest.vmid, node, e);
                }
            }
        }
    ...
    async fn scan_guest(&self, vm_id: Uuid, proxmox_vmid: u32, node: &str) -> Result<()> {
        debug!("Running Guest Agent discovery on VM {} (Proxmox VMID: {})", vm_id, proxmox_vmid);
        
        // 1. Verify Guest Agent is responsive
        if self.pve_client.vm_agent_ping(node, proxmox_vmid).await.is_err() {
            warn!("QEMU Guest Agent not responding on VM {}. Discovery skipped, but resources remain historically preserved.", proxmox_vmid);
    ...
    async fn discover_linux_services(&self, node: &str, proxmox_vmid: u32) -> Result<Vec<String>> {
        ...
        let command = "systemctl list-unit-files --type=service --no-pager --no-legend | awk '{print }'";
        
        let output = self.pve_client.vm_agent_exec_shell(node, proxmox_vmid, command).await?;
    ...
    fn reconcile_resources(&self, vm_id: Uuid, kind: &str, scanned_identifiers: Vec<String>) -> Result<()> {
        ...
        for scanned_id in scanned_identifiers {
            if let Some(res) = existing_map.remove(&scanned_id) {
                ...
                if res.state == ResourceState::Removed {
                    self.resource_repo.update_state(res.id, ResourceState::PendingUser)?;
                    ...
                    debug!("Resource {} remains unchanged in state {:?}", scanned_id, res.state);
                }
            } else {
                // New resource
        ...
        for (id_str, res) in existing_map {
            ...
            if res.state != ResourceState::Removed {
                self.resource_repo.update_state(res.id, ResourceState::Removed)?;
                self.event_repo.insert(&DiscoveryEvent {
                    id: Uuid::new_v4(),
                    vm_id,
                    resource_id: Some(res.id),
