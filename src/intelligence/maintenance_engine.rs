...
use crate::db::sqlite::repository::{
    AlertRepository, MaintenanceWindowRepository, MetricRepository, MonitorRepository,
    ResourceRepository, RuleRepository,
};
        ...
        for window in active_windows {
            match window.scope_type {
                MaintenanceScopeType::Global => {
                    debug!(
                        "Incident {} suppressed by GLOBAL maintenance window {}",
                ...
                MaintenanceScopeType::Vm => {
                    if Some(resource.vm_id) == window.scope_id {
                        debug!(
                            "Incident {} suppressed by VM maintenance window {}",
                            incident.id, window.id
                        );
                        return Ok(true);
                    }
                ...
                MaintenanceScopeType::Resource => {
                    if Some(resource.id) == window.scope_id {
                        debug!(
                            "Incident {} suppressed by RESOURCE maintenance window {}",
                            incident.id, window.id
                        );
                        return Ok(true);
                    }
                ...
                MaintenanceScopeType::Rule => {
                    if Some(rule.id) == window.scope_id {
                        debug!(
                            "Incident {} suppressed by RULE maintenance window {}",
                            incident.id, window.id
                        );
                        return Ok(true);
                    }
